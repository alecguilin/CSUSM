...
{ 
 ...
 public: // prototypes to be used by the client
  ...
  //          otherwise, throws an exception - Underflow
  // PARAMETER: provide variable to recieve a copy of the top element (pass by ref)
  void topElem(el_t&);
  
  // ** Must add good comments for each function - See Notes1B

  //PURPOSE: checks if the stack is empty returns true if empty and false if not empty
  bool isEmpty();
  //PURPOSE: checks if the stack is full returns true if full and false if not full
  bool isFull();
  //PURPOSE: displays all values in the stack
  void displayAll();
  //PURPOSE: clears all values in the stack
  void clearIt();
